  ...
  useEffect(() => {
    const fetchStages = async () => {
      ...
      const stageResponse = await getAllStages(
        parseInt(session?.userData.id as string)
      );
    ...
    <div className="fixed bottom-0 left-0 right-0 p-4 bg-slate-300 flex justify-around z-[9999]">
      {stages.length === 0 ? (
        <div className="text-sm font-mono">
          No interview stages exist. Add one by going to
          <Link href="/manage-stages" className="text-blue-600">
            {" Manage Stages"}
          </Link>
        </div>
      ) : (
        stages.map((stage) => (
          ...
          >
            <span className="text-xs border border-gray-400 rounded-xl py-1 px-2">
              {stage}
            </span>
          </div>
        ))
